    ...
    public partial class FLTD
    {
        public bool DumpData(string filePath)
        {
            using (var fp = new StreamWriter(new FileStream(filePath, FileMode.Create, FileAccess.Write)))
            {
                ...
                {
                    DumpClassicFormat(fp);
                }
            }
            return true;
        }
        private void DumpNGSFormat(StreamWriter fp)
        {
            ...
            for (int i = 0; i < data1.Length; i++)
            {
                ...
                fp.WriteLine($"value1               = [{data1[i].address3:X8}]");

                fp.Write("\n");
                fp.WriteLine($"data4                =");
                for (int k = 0; k < data1[i].data4.Length; k++)
                {
                    if (k > 0 && k % 0x16 == 0)
                        fp.Write("\n");
                    fp.Write($"{data1[i].data4[k]:X2} ");
                }
                fp.Write("\n\n");
            }
            fp.WriteLine("");
            ...
            for (int i = 0; i < data1.Length; i++)
            {
                for (int j = 0; j < data1[i].data5.Length; j++)
                {
                    ...
                    fp.Write("\n\n");
                    fp.WriteLine($"data11             =");
                    for (int k = 0; k < data1[i].data5[j].data11.Length; k++)
                    {
                        if (k > 0 && k % 0x4 == 0)
                            fp.Write("\n");
                        fp.Write($"{data1[i].data5[j].data11[k]:F6} ");
                    }
                    fp.Write("\n\n");
                }
            }
        ...
        private void DumpClassicFormat(StreamWriter fp)
        {
